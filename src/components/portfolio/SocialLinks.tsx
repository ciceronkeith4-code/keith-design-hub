...
export function SocialLinks({ className }: { className?: string }) { return <div className={cn("flex items-center gap-3", className)}>{SOCIALS.map((social) => { const Icon = icons[social.id]; return <a key={social.id} href={social.url} aria-label={social.label} {...(social.url.startsWith("mailto:") ? {} : { target: "_blank", rel: "noopener noreferrer" })} className="group flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-[#BDB5A8] transition hover:-translate-y-1 hover:border-[#E25822] hover:bg-[#E25822] hover:text-[#F8F1E7]"><Icon className="h-[17px] w-[17px]" /></a>; })}</div>; }
